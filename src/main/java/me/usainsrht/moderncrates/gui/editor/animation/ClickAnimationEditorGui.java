...
/**
 * Editor GUI for Click-type animations.
 * Settings: type, gui title, shuffling title, rows, fill, rewardSlots, fillerSlots,
 * rewardHideItem, shuffleAmount, shuffleTicks, rewardAmount, showRevealedItemsFor, sounds.
 */
public class ClickAnimationEditorGui extends EditorGuiBase {

    private final Animation animation;

    public ClickAnimationEditorGui(Player player, ModernCratesPlugin plugin, Animation animation) {
        super(player, plugin);
    ...
    public void open() {
        inventory = Bukkit.createInventory(this, 54,
                TextUtil.parse("<dark_red><bold>Click Animation: " + animation.getId()));
        fillBlack();
        ...
        inventory.setItem(22, createNumberItem("DIAMOND", "Reward Amount", animation.getRewardAmount()));
        inventory.setItem(24, createNumberItem("CLOCK", "Show Revealed For", animation.getShowRevealedItemsFor()));
        ...
        // Sounds
        inventory.setItem(34, ItemBuilder.create("NOTE_BLOCK", "<yellow><bold>Sounds",
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        switch (slot) {
            ...
            case 22 -> { adjustInt(animation::getRewardAmount, animation::setRewardAmount, rightClick, shiftClick, 1, 100); open(); }
            case 24 -> { adjustInt(animation::getShowRevealedItemsFor, animation::setShowRevealedItemsFor, rightClick, shiftClick, 1, 999); open(); }
            case 25 -> {
                if (rightClick) {
            ...
            case 34 -> new AnimationSoundsEditorGui(player, plugin, animation).open();
